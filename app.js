...
function addEmployee(){
  ...
  if (isNaN(rate) || rate < 0) { showToast("⚠ Nhập lương/đơn hợp lệ"); return; }
  ...
  employees.push({ id: uid(), name, rate });
...
function updateEmployeeRate(id, newRate){
  ...
  if (emp) {
    ...
    saveEmployees(employees);
    renderRanking();
    showToast("💾 Đã cập nhật lương/đơn");
  }
}
  ...
  employees.forEach(emp => {
    ...
    row.innerHTML = `
      <div class="item-main">
        ...
        <div class="item-sub">Lương / đơn</div>
      </div>
      <input type="number" class="mini-input" value="${emp.rate}">
      <button>Xoá</button>
    `;
    row.querySelector("input").addEventListener("change", (e) => updateEmployeeRate(emp.id, e.target.value));
...
function computeStats(){
  ...
  const stats = employees.map(emp => {
    ...
    const salary = totalQty * emp.rate;
    return { name: emp.name, rate: emp.rate, totalQty, totalRevenue, salary };
  });
    ...
    stats.forEach((s, i) => {
      ...
      row.innerHTML = `
        ...
        <div class="rank-main">
          ...
          <div class="rank-sub">Doanh thu ${formatVND(s.totalRevenue)} · ${formatVND(s.rate)}/đơn</div>
        </div>
